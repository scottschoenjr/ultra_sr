...
addParameter(p, 'DopplerStyle', false, @islogical); % SJS
addParameter(p, 'ColorLims', [], @isnumeric); % SJS
...
ax = p.Results.Parent;
plotDoppler = p.Results.DopplerStyle; % SJS
cLimits = p.Results.ColorLims; % SJS
assert(scale >= 0);
...
% SJS
if plotDoppler
    vmax = max(v, [], 'all', 'omitnan'); % SJS (Only vertical component)
    vmin = min(v, [], 'all', 'omitnan'); % SJS (Only vertical component)
    if ~isempty( cLimits )
        vmin = max( vmin, cLimits(1) );
        vmax = min( vmax, cLimits(2) );        
    end
    Ic = round( ncolors.*( (v - vmin)/( vmax - vmin ) ) );
    
     % For plotting
    Imax = max(abs(v), [], 'all', 'omitnan');
    Imin = -Imax;
end
